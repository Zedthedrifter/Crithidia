...
r2=$4
  ...
  fasta_extend.py ${infile/'fasta'/'extended.fasta'} ${infile} 150
  bowtie2-build -f ${infile/'fasta'/'extended.fasta'} mini_ref
  /usr/bin/bowtie2-align-s --wrapper basic-0 -x mini_ref -p 20 --threads 16 --phred33 --very-sensitive --quiet --time --rg-id ${strain} --rg SM:${strain} --rg PL:'ILLUMINA' -S map_mini.bam -1 ${r1} -2 ${r2}
 ...
 #CSB3 reads
  ...
  samtools view mini.mapped.sorted.bam -Sbh -f 2|bamtools filter -tag XM:'<=1'|samtools sort -o ${infile/'fasta'/'.mapped.sorted.paired.XM1.bam'}
  samtools index ${infile/'fasta'/'.mapped.sorted.paired.XM1.bam'}
...
r1=/home/zed/Datastore_map/Temp/Cf_kDNA/X204SC23072602-Z01-F001/01.RawData/Cf_kDNA/Cf_kDNA_EKDN230029884-1A_HKM2VDSX7_L1_1.fq.gz
r2=/home/zed/Datastore_map/Temp/Cf_kDNA/X204SC23072602-Z01-F001/01.RawData/Cf_kDNA/Cf_kDNA_EKDN230029884-1A_HKM2VDSX7_L1_2.fq.gz
strain=Cf
#run komics assembly and completeness assessment
  #komics_assembly ${log_file} ${r1} ${r2} ${strain} $stats_file
  completeness_assessment Cf.minicircles.fasta ${strain} ${r1} ${r2}
  #./workscripts.py tmp.Cf.circular.fasta Cf.polished.fasta
